...
namespace Zxcvbn.Matcher
{
    ...
    internal class DictionaryMatcher : IMatcher
    {
        ...
        {
            // Build full path inside Unity project
            string path = Path.Combine(Application.dataPath, "Scripts/zxcvbn/Dictionaries", wordListFile);

            // Read the file
            var lines = File.Exists(path)
                ? File.ReadAllLines(path)
                : Utility.GetEmbeddedResourceLines($"Zxcvbn.Dictionaries.{wordListFile}");

            if (lines == null)
                throw new FileNotFoundException($"Could not find dictionary file: {wordListFile} at path: {path}");
